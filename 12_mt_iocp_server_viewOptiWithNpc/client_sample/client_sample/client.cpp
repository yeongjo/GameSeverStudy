...
#include "..\..\12_mt_iocp_server_viewOptiWithNpc\st_iocp_server/protocol.h"
...
int prev_avatar_x, prev_avatar_y;
...
void client_main()
{
	...
	auto dx = (avatar.m_x - prev_avatar_x);
	auto dy = (avatar.m_y - prev_avatar_y);
	if(dx != 0 || dy != 0){
		stringstream ss;
		ss << buf;
		if (dy > 0) {
		...
		ss << ", ";
		auto offset_ms = duration_cast<milliseconds>(current_time - prev_time).count();
		if(offset_ms == 0){
			ss << "1 BlockPerMs";
		}else{
			ss << ((abs(dx)+abs(dy)) / (float)offset_ms) << " BlockPerMs";
		}
		prev_time = system_clock::now();
		text.setString(ss.str());
	}
	prev_avatar_x = avatar.m_x;
	prev_avatar_y = avatar.m_y;
